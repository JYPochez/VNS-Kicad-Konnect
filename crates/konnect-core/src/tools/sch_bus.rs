...
use crate::tool;
use crate::tools::{get_path, opt_f64, require_f64, ToolContext, ToolDef};
use konnect_sexp::{
    parser::parse_sexp,
    schematic::{
        extract_lib_pins_for_unit, extract_symbol_instances, format_bus, format_bus_entry,
        format_net_label, format_wire, pin_endpoint,
    },
    writer::write_atomic,
};
use serde_json::json;
    ...
    vec![
        ...
        tool!(
            ...
            "Add a bus entry — the 45-degree tick that connects a wire to a bus. Required: a wire \
             ...
             wire side; 'dx'/'dy' (default 2.54) give the signed offset to the bus side.",
            json!({
                "type": "object",
                "properties": {
                    "schematic": { "type": "string", "description": "Path to .kicad_sch file" },
                    ...
                    "dx": { "type": "number", "description": "Signed X offset to the bus side (default 2.54)" },
                    "dy": { "type": "number", "description": "Signed Y offset to the bus side (default 2.54)" }
                },
        ...
        tool!(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "entry_size": { "type": "number", "description": "Bus entry tick length (default 2.54)" }
                },
...
async fn handle_add_bus_entry(
    ...
    let content = std::fs::read_to_string(&sch_path)?;
    let new_content =
        crate::tools::sch_wiring::insert_before_close(&content, &format_bus_entry(x, y, dx, dy));
    write_atomic(&sch_path, &new_content)?;

    Ok(CallToolResult::json(&json!({
        "added": "bus_entry",
        "wire_side": { "x": x, "y": y },
        "bus_side": { "x": x + dx, "y": y + dy }
    })))
}
...
async fn handle_connect_pins_to_bus(
    ...
    let size = opt_f64(args, "entry_size").unwrap_or(2.54);
    ...
    for conn in &conns {
        ...
        inserts.push_str("\n  ");
        inserts.push_str(&format_bus_entry(entry_x, entry_y, dx, dy));
...
mod tests {
    ...
    #[test]
    fn bus_entry_offset_is_signed() {
        assert!(format_bus_entry(10.0, 20.0, 2.54, 2.54).contains("(size 2.54 2.54)"));
        assert!(format_bus_entry(10.0, 20.0, -2.54, -2.54).contains("(size -2.54 -2.54)"));
    }
